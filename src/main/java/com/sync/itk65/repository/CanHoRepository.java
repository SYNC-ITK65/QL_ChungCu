...
public interface CanHoRepository extends JpaRepository<CanHo, Long> {
       ...
       @Query(value = "SELECT COUNT(*) FROM can_ho", nativeQuery = true)
       long countTotalCanHo();

       // Lấy tổng số lượng căn hộ có trạng thái trống (chưa có người ở) để thống kê
       @Query(value = "SELECT COUNT(*) FROM can_ho WHERE trang_thai IS NULL OR LOWER(trang_thai) LIKE '%trống%'", nativeQuery = true)
       ...
       // quả
       @Query(value = "SELECT * FROM can_ho WHERE " +
                     "(:trangThai IS NULL OR :trangThai = '' OR LOWER(trang_thai) LIKE LOWER(CONCAT('%', :trangThai, '%'))) AND "
                     +
                     "(:dienTich IS NULL OR dien_tich = :dienTich) AND " +
                     "(:tang IS NULL OR tang = :tang)", nativeQuery = true)
